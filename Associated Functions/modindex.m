function [M] = modindex(data,plotans,N, trialnum)
...
%Generate plot of normalized gamma amplitude as a function of phase (two full cycles)
 %{
        ...
        axes1 = axes('Parent',figure,'YTick',[0 0.02 0.04 0.06 0.08],'XTick',[0 180 360 540 720]);
        xlim(axes1, [0 720]);
        ylim(axes1, [0 0.08]);
        hold(axes1,'all')
        %}
if plotans == 1
    %figure; hold on
    figureModindex = figure; hold on
    axes1 = axes('Parent',figureModindex,'YTick',[0 0.02 0.04 0.06 0.08],'XTick',[0 180 360 540 720]);
    xlim(axes1, [0 720]);
    ylim(axes1, [0 0.08]);
    hold(axes1,'all')   
    ...
    sgtitle(sprintf('Two Cycles of Normalized Gamma Amplitude by Phase, Trial %d',trialnum))
    uiwait(figureModindex)
end
end
