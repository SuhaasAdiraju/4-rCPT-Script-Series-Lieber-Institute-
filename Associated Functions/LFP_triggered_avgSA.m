...
if phase == 0
    [~, peaks] = findpeaks(filtered_samples, 'MINPEAKDISTANCE', round(MPD)); % 'MINPEAKDISTANCE',
elseif phase == 1
    [~, troughs] = findpeaks(filtered_samples.*-1,'MINPEAKDISTANCE', round(MPD)); % 'MINPEAKDISTANCE',
end
...
set(gca,'TickDir','out')
xlabel('Time (Seconds)')
sgtitle('Average LFP Snippet, for Peak or Trough Triggered Average')
end
