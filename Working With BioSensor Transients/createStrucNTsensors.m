...
%% Description
% This function is for normalizing, trimming, formatting, and combining neural activity
...
mousename = erase(Tstamps_name,'.csv')
...
TransientsOnly = ((Transients([3:end],[2:end])));
...
ROItransients = TransientsOnly(:,ROI);
ROItransients = ROItransients';

%{
ROItransientsNew = [];
for z = 1:length(ROItransientsNew(1,:))
    ROItransientsNew(1,z) = ROItransients{1,z};
end
%}
...
%% We want to normalize to a baseline or 'flatten' the transients
% OPTION IS IN SLICING STEP
%{
...
cd(saveplace);
mousename = append(mousename,'_RAW');
save(mousename,'-struct', 'savename');

sprintf('Your new structure has been saved!\nPath: ''%s'',\nName: ''%s''\nAnd also exists in the workspace for immediate use...',saveplace,mousename)
