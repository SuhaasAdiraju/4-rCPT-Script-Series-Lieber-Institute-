...
%% Load-In Existing Structure
    ...
    % Matrix for data 1
    for eventi = 1:length(data1.Events_Accepted_lfp)
        for stampi = 1:length((data1.Events_Accepted_lfp{1,1}))
            %if isempty(Event_Type{1,eventi}) == 0
            AllEventArray_data1(eventi,stampi) = data1.Events_Accepted_lfp{1,eventi}(1,stampi);
            %end
        end
    end
    ...
    if exist('data2', 'var')
        data1matrx = AllEventArray_data1;
            ...
            % Matrix for data 2
            for eventi = 1:length(data2.Events_Accepted_lfp)
                for stampi = 1:length((data2.Events_Accepted_lfp{1,1}))
                    %if isempty(Event_Type{1,eventi}) == 0
                    AllEventArray_data2(eventi,stampi) = data2.Events_Accepted_lfp{1,eventi}(1,stampi);
                    %end
                end
            end
...
if exist('data2','var')
    ...
    legend({'Randomly sampled distribution', 'Observed Value'})
