function [] = sliceNTsensors(struc_path, struc_name, srate, TimeWin, saveplace, mousename)
%% DESCRIPTION
...
if sum(size(FIRBeam_On)) >= 2 
    for i = 1:length(FIRBeam_Onidx)
        if (FIRBeam_Onidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (FIRBeam_Onidx(i)-(srate*TimeWin{1}))>(0)  
            CutTransients.FIRBeam_On_transients{i} = Transients(1,[FIRBeam_Onidx(1,i)-(srate*TimeWin{1}):FIRBeam_Onidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
...
if sum(size(FIRBeam_Off)) >= 2 
    for i = 1:(length(FIRBeam_Offidx))
        if (FIRBeam_Offidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (FIRBeam_Offidx(i)-(srate*TimeWin{1}))>(0)        
            CutTransients.FIRBeam_Off_Transients{i} = Transients(1,[FIRBeam_Offidx(1,i)-(srate*TimeWin{1}):FIRBeam_Offidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
...
if sum(size(Center_ScTouch)) >= 2 
    for i = 1:length(Center_ScTouchidx)
        if (Center_ScTouchidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Center_ScTouchidx(i)-(srate*TimeWin{1}))>(0)              
            CutTransients.Center_ScTouch_Transients{i} = Transients(1,[Center_ScTouchidx(1,i)-(srate*TimeWin{1}):Center_ScTouchidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
...
if sum(size(Start_ITI)) >= 2 
    for i = 1:length(Start_ITIidx)
        if (Start_ITIidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Start_ITIidx(i)-(srate*TimeWin{1}))>(0)                        
            CutTransients.Start_ITI_Transients{i} = Transients(1,[Start_ITIidx(1,i)-(srate*TimeWin{1}):Start_ITIidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
if sum(size(Stimulus)) >= 2 
    for i = 1:length(Stimulusidx)
        if (Stimulusidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Stimulusidx(i)-(srate*TimeWin{1}))>(0)                      
            CutTransients.Stimulus_Transients{i} = Transients(1,[Stimulusidx(1,i)-(srate*TimeWin{1}):Stimulusidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
if sum(size(Hit)) >= 2
    for i = 1:length(Hitidx)
        if (Hitidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Hitidx(i)-(srate*TimeWin{1}))>(0)                        
            CutTransients.Hit_Transients{i} = Transients(1,[Hitidx(1,i)-(srate*TimeWin{1}):Hitidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end    
if sum(size(Miss)) >= 2
    for i = 1:length(Missidx)
        if (Missidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Missidx(i)-(srate*TimeWin{1}))>(0)               
            CutTransients.Miss_Transients{i} = Transients(1,[Missidx(1,i)-(srate*TimeWin{1}):Missidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
if sum(size(Correct_Rej)) >= 2
    for i = 1:length(Correct_Rejidx)
        if (Correct_Rejidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (Correct_Rejidx(i)-(srate*TimeWin{1}))>(0)                      
            CutTransients.Correct_Rej_Transients{i} = Transients(1,[Correct_Rejidx(1,i)-(srate*TimeWin{1}):Correct_Rejidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
if sum(size(False_Alarm)) >= 2
    for i = 1:length(False_Alarmidx)
        if (False_Alarmidx(i)+(srate*TimeWin{1}))<(length(Transients)) && (False_Alarmidx(i)-(srate*TimeWin{1}))>(0)                     
            CutTransients.False_Alarm_Transients{i} = Transients(1,[False_Alarmidx(1,i)-(srate*TimeWin{1}):False_Alarmidx(1,i)+(srate*TimeWin{1})]);
        end
    end
end
